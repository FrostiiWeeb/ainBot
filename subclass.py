...
class Embed(discord.Embed):
    def __init__(self, *, color:int=0x0307fc, title:str="", description:str="", type:str="rich"):
...
class AinContext(commands.Context):
    async def reply(self, content:str=None, **kwargs) -> discord.Message:
        ...
        if not 'override_color' in kwargs: # If we shouldn't override the default embed color, then don't override it.
            if 'embed' in kwargs and isinstance(kwargs['embed'], discord.Embed) and not 'no_embed_override' in kwargs:
                now = datetime.utcnow().strftime("%m/%d/%Y %H:%M:%S")
                kwargs['embed'].color = 0x0307fc
                kwargs['embed'].set_footer(icon_url=self.author.avatar.url, text=f"Requested by {self.author} at {now}")
...
class CustomBot(commands.AutoShardedBot):
    ...
    async def on_command_error(self, ctx: commands.Context, exception: Exception, /) -> None:
        ...
        if isinstance(exception, commands.errors.CommandNotFound):
        ...
        elif isinstance(exception, commands.errors.BadArgument) or isinstance(exception, commands.errors.ArgumentParsingError):
            ...
            return await ctx.reply(embed=emb, view=views.ReportBug(self, exception, ctx.message))
    ...
    async def process_commands(self, message: discord.Message, /) -> None:
        if message.author.id in self.blacklisted_ids:
            ...
            view.add_item(discord.ui.Button(label="Appeal", url="https://discord.gg/FjfR6uDBgs"))
            emb = discord.Embed(title="Well..", description=":x: **You can't use ainBot because you're blacklisted. You can appeal by joining the discord server with the button below.**", color=0x000000)
